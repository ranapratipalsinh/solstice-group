...
export interface StaggerCard {
  tempId: number;
  testimonial: string;
  by: string;
  imgSrc?: string;
  /** If set, clicking the centered (focused) card opens this URL instead of cycling. */
  href?: string;
}

const testimonials: StaggerCard[] = [
  {
    tempId: 0,
...
const TestimonialCard: React.FC<TestimonialCardProps> = ({
  ...
  return (
    <div
      onClick={() => {
        if (isCenter && testimonial.href) {
          window.open(testimonial.href, '_blank', 'noopener,noreferrer');
        } else {
          handleMove(position);
        }
      }}
