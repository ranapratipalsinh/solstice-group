...
export function CountUpStat({
    ...
    if (variant === 'bar') {
        return (
            <div className="py-4 text-center md:py-0">
                <p ref={ref} className="font-display text-4xl font-bold text-solstice-600 sm:text-5xl">
                    {display}
                    {suffix}
                </p>
                <p className="mt-2 text-sm font-semibold uppercase tracking-[0.2em] text-solstice-700">{label}</p>
            </div>
        );
    }
    ...
    return (
        <div className="rounded-3xl bg-white p-6 text-center shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border dark:border-slate-700/60 dark:bg-solstice-800 dark:shadow-none dark:hover:border-solstice-700/60">
            <p ref={ref} className="font-display text-3xl font-semibold text-solstice-700 dark:text-white sm:text-4xl">
                {display}
                {suffix}
            </p>
