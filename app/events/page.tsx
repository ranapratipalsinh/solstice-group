    ...
    return (
        <div>
            <PageHeader
                ...
                description="Corporate events, brand launches, and exhibitions delivered end-to-end by our events subsidiary."
            />
                        ...
                        <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {upcoming.map((event, index) => (
                                <ScrollReveal key={event.slug} delayMs={((index % 3) + 1) * 200}>
                                    <article className="h-full overflow-hidden rounded-3xl border border-slate-200 bg-solstice-50 shadow-sm dark:border-slate-700/60 dark:bg-solstice-900">
                                        {event.coverImageUrl && (
                    ...
                    <div className="container">
                        ...
                        <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {past.map((event, index) => (
                                <ScrollReveal key={event.slug} delayMs={((index % 3) + 1) * 200}>
                                    <article className="h-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700/60 dark:bg-solstice-800">
                                        {event.coverImageUrl && (
                                            // eslint-disable-next-line @next/next/no-img-element
